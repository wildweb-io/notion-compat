...
import type {BaseBlock, Block as Block1} from 'notion-types';
import type {
	Block,
	BlockMap,
	Page,
...
export const convertBlock = ({
	...
	switch (block.type) {
		...
		case 'video': {
			...
			try {
				...
				switch (href.hostname) {
					...
					case 'www.loom.com': {
						href.pathname = href.pathname.replace(/^\/share\//i, '/embed/');
						compatBlock.format.display_source = href.toString();
				...
				try {
					...
					switch (href.hostname) {
						...
						case 'open.spotify.com': {
							if (
								href.pathname.includes('/embed/') ||
								href.pathname.includes('/embed-podcast/')
							) {
								break;
							}

							if (href.pathname.startsWith('/playlist/')) {
								href.pathname = `/embed${href.pathname}`;
							} else if (href.pathname.startsWith('/episode/')) {
								href.pathname = `/embed-podcast${href.pathname}`;
							}
						...
						case 'airtable.com': {
							if (!href.pathname.startsWith('/embed/')) {
								href.pathname = `/embed${href.pathname}`;
								compatBlock.format.display_source = href.toString();
							}
		...
		case 'link_preview': {
			const url = block?.link_preview?.url ?? undefined;
